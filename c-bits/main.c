            ...
            , RAddr stack_top )
{
  ...
  if (DEBUG) fprintf(stderr, "new esi      : %p\n", new_heap_ptr);
...
void initHeap(){
  if (DEBUG) fprintf(stderr, "Heap Size = %ld\n", HEAP_SIZE);
  ...
  if (DEBUG) fprintf(stderr, "ALLOC-0: %p, %p \n", HEAP, HEAP_END);

  // 8-byte align 
  HEAP = align8(HEAP);

  if (DEBUG) fprintf(stderr, "ALLOC-1: %p \n", HEAP);
